...
<script>
import { CountryGet, CustomerCreate, CustomerEdit } from '@/d/api/crm'
...
export default {
  ...
  methods: {
    ...
    async CustomerCreate() {
      const { message } = await CustomerCreate(this.formData)
      successMessage(this, '成功', message)
      this.close()
      this.$emit('refreshTable', true)
    },
    async CustomerEdit() {
      const { message } = await CustomerEdit(this.formData)
      successMessage(this, '成功', message)
      this.close()
      this.$emit('refreshTable', true)
    }
